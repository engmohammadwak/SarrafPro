        ...
        <table>
            ...
            <tr>
                ...
                <td style="color:var(--text-muted);font-size:13px">
                    {{ $a->shop->city ?? '-' }}
                    @if($a->shop?->country)<br><span style="font-size:11px">{{ $a->shop->country }}</span>@endif
                </td>
                ...
                <td>
                    <div style="display:flex;gap:6px;flex-wrap:wrap">
                        ...
                        <a href="{{ route('agent.shops.show', $a->id) }}"
                           style="display:inline-flex;align-items:center;gap:5px;padding:6px 12px;background:#f4f6fb;border:1px solid #e5e7eb;border-radius:8px;font-size:12px;font-weight:600;color:var(--text-dark);text-decoration:none"
                           onmouseover="this.style.background='#eef0f7'" onmouseout="this.style.background='#f4f6fb'">
                            <i class="fas fa-eye"></i> تفاصيل
                        </a>

                        {{-- زر المعاملات --}}
                        <a href="{{ route('agent.shops.transactions', $a->id) }}"
                           style="display:inline-flex;align-items:center;gap:5px;padding:6px 12px;background:#eff6ff;border:1px solid #bfdbfe;border-radius:8px;font-size:12px;font-weight:600;color:#1d4ed8;text-decoration:none"
                        ...
                        @if($a->link_status === 'approved')
                            ...
                            <form method="POST" action="{{ route('agent.shops.block', $a->id) }}" style="display:inline" onsubmit="return confirm('هل تريد توقيف هذا المحل؟')">
                                @csrf @method('PATCH')
                                <button type="submit" style="display:inline-flex;align-items:center;gap:5px;padding:6px 12px;background:#fff3cd;border:1px solid #ffc10744;border-radius:8px;font-size:12px;font-weight:600;color:#92610a;cursor:pointer;font-family:Tajawal,sans-serif"
                            ...
                            <form method="POST" action="{{ route('agent.shops.unblock', $a->id) }}" style="display:inline">
                                @csrf @method('PATCH')
                                <button type="submit" style="display:inline-flex;align-items:center;gap:5px;padding:6px 12px;background:#d1fae5;border:1px solid #10b98144;border-radius:8px;font-size:12px;font-weight:600;color:#065f46;cursor:pointer;font-family:Tajawal,sans-serif"
                            ...
                            <form method="POST" action="{{ route('agent.shops.unlink', $a->id) }}" style="display:inline"
                                ...
                                @csrf @method('PATCH')
                                <button type="submit" style="display:inline-flex;align-items:center;gap:5px;padding:6px 12px;background:#fee2e2;border:1px solid #ef444444;border-radius:8px;font-size:12px;font-weight:600;color:#991b1b;cursor:pointer;font-family:Tajawal,sans-serif"
